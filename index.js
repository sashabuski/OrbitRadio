...
async function fetchStationsFromAPI(limit = 500000) {
    try {
        const response = await fetch(`http://localhost:9000/.netlify/functions/api/stations`); // Fetch from API
        if (!response.ok) {
...
async function getLocalStations(){
    ...
    try {
        const response = await fetch("http://localhost:9000/.netlify/functions/api/stations");
        const stations = await response.json();
